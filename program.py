...
width=827, height=476, min_size=(767, 350), on_close=lambda: dpg.hide_item("plugin_node_editor_registry")):
    ...
    with dpg.group(horizontal=True, horizontal_spacing=0):
        ...
        dpg.add_button(label="Добавить", callback=inf.open_add_plugin_window)
        dpg.add_button(label="Дублировать", enabled=False)
        dpg.add_spacer(width=10)
        dpg.add_image("vertical_separator")
        dpg.add_spacer(width=10)
        dpg.add_button(label="Загрузить")
        dpg.add_button(label="Сохранить", enabled=False, tag="save_to_file_button")
        dpg.add_spacer(width=10)
        dpg.add_image("vertical_separator")
        dpg.add_spacer(width=10)
