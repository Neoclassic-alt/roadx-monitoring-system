...
def edit_image(img, parameters, **kwargs):
    ...
    mask = cv2.inRange(hsv_img, *get_boundaries_hsv(parameters['color']))
    ...
    if not rectangle is None:
        ...
        traffic_light_image = img[rectangle[0]+2:rectangle[2]-2, rectangle[1]+2:rectangle[3]-2]
        ...
        if parameters["write_to_image"] and not calculated_color is None:
            cv2.putText(img, calculated_color, (rectangle[0] - 90, rectangle[3] - 20),
            cv2.FONT_HERSHEY_DUPLEX, 1, (255, 0, 0), 1)
    """
    for i, c in enumerate(contours):
        contours_poly[i] = cv2.approxPolyDP(c, 3, True)
        boundRect[i] = cv2.boundingRect(contours_poly[i])
        if boundRect[i][2] > 1 and boundRect[i][3] > 1 and i == parameters["traffic_light_number"]:
            ...
            if parameters["write_to_image"] and not calculated_color is None:
                cv2.putText(img, calculated_color, (boundRect[i][0]- 90, boundRect[i][1]+boundRect[i][3] - 20),
                cv2.FONT_HERSHEY_DUPLEX, 1, (255, 0, 0), 1)"""
    ...
    if not frame_index is None and series != 0:
        return {'image': img, "video_data": (frame_index/frame_rate, 1, series)}
    else:
        return {'image': img}
